...
__device__ void TraceRay(PathTracerparams& params, Ray& ray, RayPayload& payload) {
    ...
    for (int i = 0; i < params.scene_size; ++i) {
        if (params.scene[i]->IntersectionTestPtr(*params.scene[i], ray, attr)) closet_hit_obj = params.scene[i];
    }
    if (closet_hit_obj != nullptr) {
        closet_hit_obj->ClosetHitPtr(*closet_hit_obj, ray, payload, attr);
    } else {
...
__global__ void SamplePixel(PathTracerparams params) {
    ...
    curand_init(x * params.width + y, 0, 0, &(params.d_rng_states[x * params.width + y]));
    for (uint i = 0; i < params.spp; ++i) {
        Ray ray = params.camera->RayGen(x, y, &(params.d_rng_states[x * params.width + y]));
        RayPayload payload;
        ...
        while (payload.recursion_depth < params.max_recursion_depth) {
            ...
            printf("[%d, %d] %d\n", x, y, payload.recursion_depth);
        }
    }
    params.output_buffer_gpu_handle[x * params.width + y + 0] = radiance.x / float(params.spp);
    params.output_buffer_gpu_handle[x * params.width + y + 1] = radiance.y / float(params.spp);
    params.output_buffer_gpu_handle[x * params.width + y + 2] = radiance.z / float(params.spp);
}
...
void PathTracer::DispatchRay(uint8_t* buf, int size, int64_t t) {
    ...
    if (error != 0) printf("[ERROR]Cuda Error %d\n", error);
}
